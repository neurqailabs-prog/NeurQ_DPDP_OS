@echo off
REM ========================================
REM Policy Engine - Windows Setup Installer
REM ========================================
REM Individual installer for Policy Engine

setlocal enabledelayedexpansion

:: Configuration
set "PRODUCT_NAME=Policy Engine"
set "PRODUCT_SHORT=Policy-Engine"
set "VERSION=1.0.0"
set "INSTALL_DIR=%LOCALAPPDATA%\NeurQ\Policy-Engine"
...
if exist "policy-engine\target\policy-engine-1.0.0.jar" (
    copy "policy-engine\target\policy-engine-1.0.0.jar" "%INSTALL_DIR%\" >nul 2>&1
...
if exist "splash-screens\policy-engine-splash.html" (
    copy "splash-screens\policy-engine-splash.html" "%INSTALL_DIR%\splash-screen.html" >nul 2>&1
...
if exist "icons\policy-engine-icon.ico" (
    copy "icons\policy-engine-icon.ico" "%INSTALL_DIR%\%PRODUCT_SHORT%-Icon.ico" >nul 2>&1
...
echo if exist "%%INSTALL_DIR%%policy-engine-1.0.0.jar" ^(
echo     java --module-path "%%INSTALL_DIR%%\lib" --add-modules javafx.controls,javafx.fxml -jar "%%INSTALL_DIR%%policy-engine-1.0.0.jar"
echo ^) else ^(
