@echo off
REM ========================================
REM QS-PII Scanner - Windows Setup Installer
REM ========================================
REM Individual installer for QS-PII Scanner

setlocal enabledelayedexpansion

:: Configuration
set "PRODUCT_NAME=QS-PII Scanner"
set "PRODUCT_SHORT=QS-PII-Scanner"
set "VERSION=1.0.0"
set "INSTALL_DIR=%LOCALAPPDATA%\NeurQ\QS-PII-Scanner"
...
if exist "qs-pii-scanner\target\qs-pii-scanner-1.0.0.jar" (
    copy "qs-pii-scanner\target\qs-pii-scanner-1.0.0.jar" "%INSTALL_DIR%\" >nul 2>&1
    echo   ✅ Application JAR installed
)

:: Step 3: Copy Python scripts
if exist "qs-pii-scanner\src\main\python" (
    xcopy /E /I /Y "qs-pii-scanner\src\main\python\*" "%INSTALL_DIR%\python\" >nul 2>&1
...
if exist "splash-screens\qs-pii-scanner-splash.html" (
    copy "splash-screens\qs-pii-scanner-splash.html" "%INSTALL_DIR%\splash-screen.html" >nul 2>&1
...
if exist "icons\qs-pii-scanner-icon.ico" (
    copy "icons\qs-pii-scanner-icon.ico" "%INSTALL_DIR%\%PRODUCT_SHORT%-Icon.ico" >nul 2>&1
...
echo if exist "%%INSTALL_DIR%%qs-pii-scanner-1.0.0.jar" ^(
echo     java -jar "%%INSTALL_DIR%%qs-pii-scanner-1.0.0.jar"
echo ^) else ^(
