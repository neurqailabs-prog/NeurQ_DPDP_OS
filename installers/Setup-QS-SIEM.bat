@echo off
REM ========================================
REM QS-SIEM - Windows Setup Installer
REM ========================================
REM Individual installer for QS-SIEM

setlocal enabledelayedexpansion

:: Configuration
set "PRODUCT_NAME=QS-SIEM"
set "PRODUCT_SHORT=QS-SIEM"
set "VERSION=1.0.0"
set "INSTALL_DIR=%LOCALAPPDATA%\NeurQ\QS-SIEM"
...
if exist "qs-siem\target\qs-siem-1.0.0.jar" (
    copy "qs-siem\target\qs-siem-1.0.0.jar" "%INSTALL_DIR%\" >nul 2>&1
...
if exist "qs-siem\target\release\qs-siem.exe" (
    copy "qs-siem\target\release\qs-siem.exe" "%INSTALL_DIR%\" >nul 2>&1
...
if exist "splash-screens\qs-siem-splash.html" (
    copy "splash-screens\qs-siem-splash.html" "%INSTALL_DIR%\splash-screen.html" >nul 2>&1
...
if exist "icons\qs-siem-icon.ico" (
    copy "icons\qs-siem-icon.ico" "%INSTALL_DIR%\%PRODUCT_SHORT%-Icon.ico" >nul 2>&1
...
echo if exist "%%INSTALL_DIR%%qs-siem.exe" ^(
echo     start "" "%%INSTALL_DIR%%qs-siem.exe"
echo ^) else if exist "%%INSTALL_DIR%%qs-siem-1.0.0.jar" ^(
echo     java -jar "%%INSTALL_DIR%%qs-siem-1.0.0.jar"
echo ^) else ^(
